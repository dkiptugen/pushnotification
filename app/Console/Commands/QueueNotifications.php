...
class QueueNotifications extends Command
{
    ...
    public function handle()
    {
        ...
        if($stories != null){
            ...
            if($stories->flag == 0){
                if(($stories->offset + 20000) >= $total_guests){

                    $guests = Guest::skip($stories->offset)->take($total_guests - $stories->offset)->get();

                    foreach($guests as $guest){
                        Notification::send($guest, new PushNotifications($stories));
                    }
                    ...
                    $guests = Guest::skip($stories->offset)->take(20000)->get();
                    ...
                    $stories->offset = $stories->offset + 20000;
                    $stories->save();
